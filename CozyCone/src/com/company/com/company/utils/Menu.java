...
public class Menu {
    ...
    public IShapeCone checkIn(){
        ...
        if(resp == 1)
            cone = factory.reservarCone(CONE_SIMPLES);
        else
    ...
    private IShapeCone getPartyCone(IShapeCone cone){
        ...
        if(resp.equals("sim") || resp.equals("s")){
            ...
            if(resposta == 1)
                cone = new PartyCone(cone,  CONE_SIMPLES);
            else
                cone = new PartyCone(cone, CONE_COM_VARANDA);
        }
        return cone;
    }
